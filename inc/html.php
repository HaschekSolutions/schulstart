...
class HTML {
    ...
    function menu($arr, $id = "", $class = "")
    {
        ...
        foreach ($arr as $key => $val)
        {
            ...
            {
                $o.= '<li class="menu_item ' . $c . '" page="'.strtolower($key).'" id="page_' . strtolower($key) . '"><a href="' . DS . strtolower($key) . '" class="'.$val['class'].'"><center>' . $val['text'] . '</center></a></li>';
            }
        }
        $o.= '</ul>';
        return $o;
    ...
    function table($data,$header=1,$style=1)
    {
        if (!is_array($data))
            return false;
        switch($style)
        ...
        $t = '<div class="table-responsive"><table class="' . $class . '">';
    ...
    */
    ...
    {
        return '<div class="alert alert-dismissable alert-info">
                    <button type="button" class="close" data-dismiss="alert">×</button>
                    '.($title?'<h4>'.$title.'</h4>':'').'
                    <p>'.$msg.'</p>
                </div>';
    ...
    function link($text, $path, $prompt = null, $confirmMessage = "Bist du sicher?",$class="btn btn-success")
    {
...
//         $Text = preg_replace("/\[php\](.+?)\[\/php\]/is",$phpLayout, $Text); 
    ...
    function getProgressBar($percent,$alternatetext="",$width='100%',$id="")
    {
        ...
        return '<div class="progress progress-striped active">
                <div class="progress-bar" role="progressbar" aria-valuenow="'.$percent.'" aria-valuemin="0" aria-valuemax="100" style="width: '.$percent.'%;"><span class="">'.$alternatetext.'</span></div>
              </div>';
...
//              </div>';
    ...
    function span($text,$size=false)
    {
        return '<span style="'.($size?'font-size:'.$size.'px;':'').'">'.$text.'</span>';
    }
